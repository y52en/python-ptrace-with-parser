...
class SyscallParser(FunctionCall):
    ...
    def set_const(self):
        match self.arch:
            case Arch.X86_64:
                from ptrace.syscall.linux.x86_64 import (
                    SYSCALL_NAMES,
                    SOCKET_SYSCALL_NAMES,
                )
            case Arch.I386:
                from ptrace.syscall.linux.i386 import (
                    SYSCALL_NAMES,
                    SOCKET_SYSCALL_NAMES,
                )
            case Arch.PPC64:
                from ptrace.syscall.linux.powerpc64 import (
                    SYSCALL_NAMES,
                    SOCKET_SYSCALL_NAMES,
                )
            case Arch.PPC32:
                from ptrace.syscall.linux.powerpc32 import (
                    SYSCALL_NAMES,
                    SOCKET_SYSCALL_NAMES,
                )
            case Arch.AARCH64:
                from ptrace.syscall.linux.aarch64 import (
                    SYSCALL_NAMES,
                    SOCKET_SYSCALL_NAMES,
                )
            case Arch.RISCV32:
                from ptrace.syscall.linux.riscv32 import (
                    SYSCALL_NAMES,
                    SOCKET_SYSCALL_NAMES,
                )
            case Arch.RISCV64:
                from ptrace.syscall.linux.riscv64 import (
                    SYSCALL_NAMES,
                    SOCKET_SYSCALL_NAMES,
                )
            case Arch.RUNNING_BSD:
                from ptrace.syscall.freebsd_syscall import (
                    SYSCALL_NAMES,
                    SOCKET_SYSCALL_NAMES,
                )
            case _:
