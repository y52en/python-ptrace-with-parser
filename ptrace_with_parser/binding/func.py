...
from ptrace_with_parser.os_tools import RUNNING_LINUX, RUNNING_BSD, RUNNING_OPENBSD
from ptrace_with_parser.cpu_info import CPU_64BITS, CPU_WORD_SIZE, CPU_POWERPC, CPU_AARCH64, CPU_RISCV

if RUNNING_OPENBSD:
    from ptrace_with_parser.binding.openbsd_struct import (
        reg as ptrace_registers_t,
        fpreg as user_fpregs_struct)

elif RUNNING_BSD:
    from ptrace_with_parser.binding.freebsd_struct import (
        reg as ptrace_registers_t)

elif RUNNING_LINUX:
    from ptrace_with_parser.binding.linux_struct import (
        user_regs_struct as ptrace_registers_t,
        user_fpregs_struct, siginfo, iovec_struct)
    if not CPU_64BITS:
        from ptrace_with_parser.binding.linux_struct import user_fpxregs_struct
else:
...
try:
    ...
    from ptrace_with_parser.ctypes_libc import libc
